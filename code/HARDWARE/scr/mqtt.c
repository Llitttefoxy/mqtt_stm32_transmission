...
uint8_t P_TOPIC_NAME[128]={0}; // 外部变量声明，发布的主题
...
char g_connectPackFlag; // CONNECT报文状态   1：CONNECT报文成功
...
void MQTT_Buff_Init(void)
{
	...
	g_connectPackFlag = SubcribePack_flag = 0; // 各个参数清零
}
/*----------------------------------------------------------*/
...
void AliIoT_Parameter_Init(void)
{
	...
	sprintf(Username, "%s", "MQTT"); // 构建用户名，并存入缓冲区
	Username_len = strlen(Username); // 计算用户名的长度

	memset(Passward, 0, 128);		   // 用户名的缓冲区全部清零
	sprintf(Passward, "%s", "MQTTPW"); // 构建密码，并存入缓冲区
	Passward_len = strlen(Passward);   // 计算密码的长度

	memset(ServerName, 0, 128);
	sprintf((char *)ServerName, "%s", "kb5fbf25.ala.asia-southeast1.emqxsl.com"); // 构建服务器域名
	ServerPort = 1883;						  // 服务器端口号6002

	u1_printf("服 务 器：%s:%d\r\n", ServerName, ServerPort); // 串口输出调试信息
	u1_printf("客户端ID：%s\r\n", ClientID);				// 串口输出调试信息
	u1_printf("用 户 名：%s\r\n", Username);				// 串口输出调试信息
	u1_printf("密    码：%s\r\n", Passward);				// 串口输出调试信息
}
